...
export function PurchasesTab() {
  ...
  const load = useCallback(async () => {
    const [offerResult, orderResult] = await Promise.all([
      supabase.from('supplier_offers').select('id, supplier_id, unit_price, stock_quantity, lead_time_days, minimum_quantity, valid_until, supplier_product_mappings!inner(product_type, product_model, supplier_sku, pack_quantity), suppliers!inner(name, currency, order_mode, minimum_order_value)').eq('active', true).order('unit_price'),
    ...
    if (offerResult.error || orderResult.error) setMessage((offerResult.error ?? orderResult.error)?.message ?? null);
  ...
  return <div className="space-y-5 py-5">
    ...
    <div className="grid gap-5 xl:grid-cols-[minmax(0,1fr)_360px]">
      <section className="space-y-3"><div className="flex items-center justify-between gap-3"><h2 className="font-semibold">Ofertas disponíveis</h2><Input aria-label="Buscar ofertas" className="max-w-xs" placeholder="Produto, SKU ou fornecedor" value={query} onChange={(e) => setQuery(e.target.value)}/></div>
        {filtered.length === 0 ? <Card><CardContent className="py-10 text-center text-sm text-muted-foreground">Ainda não há ofertas disponíveis. Os preços aparecem após o fornecedor ser habilitado e sincronizado.</CardContent></Card> : <div className="grid gap-3 md:grid-cols-2">{filtered.map((offer) => { const mapping = offer.supplier_product_mappings; const quantity = cart[offer.id] ?? 0; return <Card key={offer.id} className={quantity ? 'border-primary/50' : ''}><CardHeader className="pb-2"><div className="flex items-start justify-between gap-2"><div><CardTitle className="text-base">{mapping.product_model}</CardTitle><p className="text-xs text-muted-foreground">{offer.suppliers.name} · SKU {mapping.supplier_sku}</p></div><Badge variant="outline">{mapping.product_type === 'inverter' ? 'Inversor' : mapping.product_type === 'battery' ? 'Bateria' : 'Acessório'}</Badge></div></CardHeader><CardContent><div className="flex items-end justify-between gap-3"><div><p className="text-lg font-semibold">{money(offer.unit_price, offer.suppliers.currency)}</p><p className="text-xs text-muted-foreground">{offer.stock_quantity == null ? 'Estoque sob consulta' : `${offer.stock_quantity} em estoque`}{offer.lead_time_days != null ? ` · ${offer.lead_time_days} dias` : ''}</p></div><div className="flex items-center rounded-lg border"><Button variant="ghost" size="icon-sm" aria-label="Diminuir" disabled={!quantity} onClick={() => changeQuantity(offer, quantity - 1)}><Minus className="h-3.5 w-3.5"/></Button><span className="w-8 text-center text-sm">{quantity}</span><Button variant="ghost" size="icon-sm" aria-label="Aumentar" onClick={() => changeQuantity(offer, Math.max(offer.minimum_quantity, quantity + 1))}><Plus className="h-3.5 w-3.5"/></Button></div></div></CardContent></Card>; })}</div>}
      </section>

...
type Supplier = { id: string; name: string; slug: string; active: boolean; ordering_enabled: boolean; order_mode: string; currency: string; minimum_order_value: number; description: string | null; is_default_for_all: boolean };
...
export function SuppliersEditor() {
  ...
  const [supplierForm, setSupplierForm] = useState({ name: '', slug: '', active: true, ordering_enabled: false, order_mode: 'quote', minimum_order_value: 0, description: '', is_default_for_all: false });
  ...
  function selectSupplier(id: string) {
    ...
    if (supplier) setSupplierForm({ name: supplier.name, slug: supplier.slug, active: supplier.active, ordering_enabled: supplier.ordering_enabled, order_mode: supplier.order_mode, minimum_order_value: supplier.minimum_order_value, description: supplier.description ?? '', is_default_for_all: supplier.is_default_for_all });
  ...
  return <div className="space-y-4">
    ...
    <div className="grid gap-4 xl:grid-cols-[280px_minmax(0,1fr)]">
      <Card><CardHeader><CardTitle className="text-base">Fornecedores</CardTitle></CardHeader><CardContent className="space-y-2">
        <Button variant={!selectedId ? 'default' : 'outline'} className="w-full justify-start" onClick={() => { setSelectedId(''); setSupplierForm({ name: '', slug: '', active: true, ordering_enabled: false, order_mode: 'quote', minimum_order_value: 0, description: '', is_default_for_all: false }); }}>Novo fornecedor</Button>
        {suppliers.map((supplier) => <button key={supplier.id} onClick={() => selectSupplier(supplier.id)} className={`w-full rounded-lg border p-3 text-left text-sm ${selectedId === supplier.id ? 'border-primary bg-primary/5' : ''}`}><span className="font-medium">{supplier.name}</span><span className="mt-1 flex flex-wrap gap-1"><Badge variant={supplier.active ? 'default' : 'secondary'}>{supplier.active ? 'Ativo' : 'Inativo'}</Badge>{supplier.ordering_enabled && <Badge variant="outline">Pedidos</Badge>}{supplier.is_default_for_all && <Badge variant="outline">Padrão</Badge>}</span></button>)}
      </CardContent></Card>

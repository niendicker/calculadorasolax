...
type Supplier = { id: string; name: string; slug: string; active: boolean; ordering_enabled: boolean; order_mode: string; currency: string; minimum_order_value: number; description: string | null; is_default_for_all: boolean; supports_partner_orders: boolean; email: string | null };
...
export function SuppliersEditor() {
  ...
  const [supplierForm, setSupplierForm] = useState({ name: '', slug: '', active: true, ordering_enabled: false, order_mode: 'quote', minimum_order_value: 0, description: '', is_default_for_all: false, supports_partner_orders: false, email: '' });
  ...
  async function saveSupplier() {
    ...
    const payload = { ...supplierForm, currency: 'BRL', description: supplierForm.description || null, email: supplierForm.email.trim() || null };
  ...
  return <div className="space-y-4">
    ...
    <div className="grid gap-4 xl:grid-cols-[280px_minmax(0,1fr)]">
      <Card><CardHeader><CardTitle className="text-base">Fornecedores</CardTitle></CardHeader><CardContent className="space-y-2">
        <Button variant={!selectedId ? 'default' : 'outline'} className="w-full justify-start" onClick={() => { setSelectedId(''); setSupplierForm({ name: '', slug: '', active: true, ordering_enabled: false, order_mode: 'quote', minimum_order_value: 0, description: '', is_default_for_all: false, supports_partner_orders: false, email: '' }); }}>Novo fornecedor</Button>

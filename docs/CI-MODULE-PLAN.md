...
**Status:** motor de cálculo completo (Fases 1-5) em `supabase/functions/_shared/commercial-industrial/`, branch `feat/ci-module`; Fase 7 (Edge Function, rota Next.js, migração, repositório admin, `CiBessProductsEditor.tsx`) completa — cadastro, edição e ativação/desativação de produtos C&I pela UI de admin, sem exclusão permanente (seção 4.3/10); falta cadastrar ao menos um produto ativo em cada ambiente antes do módulo ser considerado demonstrável (isso é operacional, não código). Fase 6: `ProjectWorkspaceShell` extraído e reutilizado, camada de dados C&I (tipos/repositório/slice Zustand) pronta, fatia mínima de UI no ar (criar/abrir/salvar/excluir um projeto C&I a partir de "Projetos") com três painéis: identificação (Visão geral), "Configuração BESS" (`CiConfigurationPanel.tsx` — seleção de produto do catálogo ativo + quantidade fixa/faixa automática de módulos) e "Curva de carga" (`CiLoadCurvePanel.tsx` — import de CSV, normalização via `parseLoadCurveCsv`, resumo via `summarizeLoadCurve`) — faltam os painéis de tarifa, estratégia e resultados. `tsconfig.json` ganhou `allowImportingTsExtensions: true` para permitir que o app importe os módulos Deno-style de `supabase/functions/_shared/commercial-industrial/` (necessário a partir deste painel).  

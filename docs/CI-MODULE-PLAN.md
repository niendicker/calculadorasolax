...
**Status:** motor de cálculo completo (Fases 1-5) em `supabase/functions/_shared/commercial-industrial/`, branch `feat/ci-module`; Fase 7 (Edge Function, rota Next.js, migração, repositório admin, `CiBessProductsEditor.tsx`) completa — cadastro, edição e ativação/desativação de produtos C&I pela UI de admin, sem exclusão permanente (seção 4.3/10); falta cadastrar ao menos um produto ativo em cada ambiente antes do módulo ser considerado demonstrável (isso é operacional, não código). Fase 6: `ProjectWorkspaceShell` extraído e reutilizado, camada de dados C&I (tipos/repositório/slice Zustand) pronta, workspace com seis painéis: identificação (Visão geral — agora um dashboard como o residencial, ver `CiOverviewPanel.tsx`: card "Instalação" com Nome/Cliente/Endereço em `EditableSummaryRow`s clicáveis que abrem `ProjectInfoModal` (extraído de `ProjectWorkspace.tsx` para `ProjectInfoModal.tsx`, ao lado de `ProjectInfoEditor.tsx`, mesma razão) para projetos já salvos — projeto novo/não salvo ainda usa o `ProjectInfoEditor` completo, já que é como o projeto ganha o primeiro nome/save; grid "Configuração do estudo" com um card por área (BESS/Curva/Tarifa/Estratégia, cada um com estado configurado/pendente e resumo, clicável para a aba correspondente); card "Resultado atual" (métricas do cenário recomendado ou CTA para calcular) — espelhando o `section === 'overview'` de `ProjectWorkspace.tsx` sem compartilhar os componentes privados dele), "Configuração BESS" (`CiConfigurationPanel.tsx` — produto do catálogo ativo + quantidade fixa/faixa automática de módulos), "Curva de carga" (`CiLoadCurvePanel.tsx` — import de CSV via `parseLoadCurveCsv`/`summarizeLoadCurve`), "Tarifa" (`CiTariffPanel.tsx`), "Estratégia" (`CiStrategyPanel.tsx` — Peak Shaving/Load Shifting/Híbrido, ranking, premissas financeiras) e "Resultados" (`CiResultsPanel.tsx` — chama `POST /api/projects/:projectId/calculations`, mostra baseline, cenário recomendado já materializado e a tabela comparativa por quantidade de módulos; exige projeto salvo, já que a rota sempre recalcula a partir do `calculation_options` salvo, não de estado local não salvo). Fase 6 **completa**: o Memorial (PDF, item 8) foi implementado em `components/app/project-workspace/ci/ci-memorial-pdf.tsx` (`@react-pdf/renderer`, mesmo padrão de `project-quote-pdf.tsx`, cujas cores de marca (`COLORS`) passaram a ser exportadas para reuso) — recebe o `CommercialIndustrialResult` já calculado (nunca recalcula), com todas as seções da seção 9 (resumo executivo, dados do projeto, premissas, BESS recomendado, estratégia, gráficos SVG de carga original×após BESS/carga-descarga/SOC, tabela e gráfico de comparação por módulos, CAPEX/economia/payback/ROI/NPV, fluxo de caixa, insights, limitações/avisos e memorial resumido com `engineVersion`/`inputFingerprint`). Botão "Gerar memorial (PDF)" em `CiResultsPanel.tsx`, habilitado quando `result.selected` existe; busca o produto do catálogo sob demanda (`listActiveCiBessProducts`) só ao gerar o PDF. `tsconfig.json` ganhou `allowImportingTsExtensions: true` para permitir que o app importe os módulos Deno-style de `supabase/functions/_shared/commercial-industrial/`.  
